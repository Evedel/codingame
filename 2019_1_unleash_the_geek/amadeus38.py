...
radarcarrierfollowers = [[-1,-1],[-1,1],[1,1],[1,-1]]
    ...
    if (numcurradars <= 3):
        ...
        while (niter < 20):
            ...
            if (not is_in_blacklist(tar[0],tar[1])):
                mindist = 1000
                for er in curradars:
                    dist = norm1(tar[0]-er[0],tar[1]-er[1])
                    if dist < mindist:
                        mindist = dist
                for bc in borderconditions:
                    dist = norm1(tar[0]-bc[0],tar[1]-bc[1])
                    if dist < mindist:
                        mindist = dist
                if (mindist > maxdist):
                    globtar  = tar[:]
            ...
            if (gamecycle < 150):
                if (prefermore):
                    ...
                    if (cells[ixy][2] == '1'):
                        mult = 1.0
                    dstloc = mult*dstloc
                if (preferenoholes) and (gamecycle < 150):
...
def get_radar_follower(i,irc):
    ...
    if (irc != -1):
        ...
        if (target[irc][0] == -1):
            tar = myrobots[irc][:]
        if (i > irc):
            tar[0] += radarcarrierfollowers[i-1][0]
            tar[1] += radarcarrierfollowers[i-1][1]
        else:
            tar[0] += radarcarrierfollowers[i][0]
            tar[1] += radarcarrierfollowers[i][1]
        disp(i,"Following a radar carrier to "+str(tar))
        noTarget = False
    return tar,noTarget
...
def handle_suicide(i):
    ...
    for ib in range(len(blacklist)):
        b = blacklist[ib]
        if (norm1(r[0]-b[0],r[1]-b[1]) <= 1):
            thebomb = b[:]
            thebombid = ib
    if (thebombid != -1):
        disp(i,"There is a bomb nearby "+str(thebomb))
        falive = 0
        ealive = 0
    ...
    for i in range(len(myrobots)):
        ...
        if (myx == -1) and (myy == -1):
            ...
            if (not handled) and (radar_cooldown == 0) \
                ...
                and (len(curradars) < len(radarcells)) and (gamecycle < 175):
