...
def handle_moving_digging(i):
    if (target[i] == None):
        ...
        if (dist > 1):
            ...
            stepx = steppoint[0]
            stepy = steppoint[1]
            print("MOVE "+str(stepx)+" "+str(stepy))
            nextstep[i] = None
        else:
...
def make_single_step(i,endpoint):
    ...
    if (norm1(tx-mx,ty-my) <=4):
        disp(i,"Next step is ["+str(tx)+ " "+str(tx)+"] "+str(globdist))
        nextstep[i] = xy2i(tx,ty)
        return endpoint
    else:
        ...
        disp(i,"Next step is ["+str(tar[0])+ " "+str(tar[1])+"] "+str(globdist))
        nextstep[i] = xy2i(tar[0],tar[1])
        return tar
