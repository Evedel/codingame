...
target  = [-1,-1,-1,-1,-1]
nextstep = [None,None,None,None,None]
myrobots = [-1,-1,-1,-1,-1]
...
minCLAnewRadar = 10
maxRuns = 1
initialrun = [maxRuns,maxRuns,maxRuns,maxRuns,maxRuns]
firstbombrunner  = 0
firstradarrunner = 1
maxinitialradars = maxRuns
maxinitialbomber = maxRuns
...
preferenoholes   = [False,False,False,False,False]
prefermore       = [False,False,False,False,False]
preferenemyholes = [True,True,True,True,True]

agressivekiller = True
killerid = -1
enemyusedmybombs = False
...
def get_radar_position_random(i):
    ...
    if (numcurradars <= 3):
        if (numcurradars == 1):
            ...
            globtar = xy2i(random.randint(11,12),random.randint(2,3))
        elif (numcurradars == 3):
            globtar = xy2i(random.randint(11,12),random.randint(11,12))
    else:
        while (niter < 20):
...
def is_radars_too_close(i):
    t = target[i]
    dist = 1000
    for cr in curradars:
        d = norm1(cells[t].x-cr[0],cells[t].y-cr[1])
        if (d < dist):
            dist = d
    if (dist <= 4):
        return True
...
def get_bomb_position_random(i):
    ...
    mx = myrobots[i][0]
    my = myrobots[i][1]
    mx1 = mx - 2 if (mx >= 3) else 1
    mx2 = mx + 2 if (mx <= 27) else 29
    my1 = my - 2 if (my >= 2) else 0
    my2 = my + 2 if (my <= 12) else 14
    if (mx1 < 0):
        mx1 = 1
        mx2 = 3
    while (noTarget) and (niter < maxiter):
        ...
        tarx = random.randint(mx1,mx2)
        tary = random.randint(my1,my2)
        if (not is_blacklist(tarx,tary)):
            noTarget = False
...
def get_bomb_position_deposit(i,only_enemy_holes):
    noTarget = True
    tar = None
    k = 0
    gdist = 1000
    ...
    for k in range(height*width):
        if ((cells[k].ore == '2') or (cells[k].ore == '3')) \
            and (cells[k].hole) and (not cells[k].blacklist):
                if (only_enemy_holes):
                    if (not cells[k].myhole):
...
def get_bomb_position_radar(i):
    ...
    mx = myrobots[i][0]
    my = myrobots[i][0]
    for k in range(len(curradars)):
        ixy = xy2i(curradars[k][0],curradars[k][1])
        if (not cells[ixy].mybomb):
            dist = norm1(mx-cells[ixy].x,my-cells[ixy].y)
            if (dist < gdist):
                gdist = dist
                noTarget = False
                tar = ixy
    if (noTarget == True):
        disp(i,"Radar position for bomb is impossible")
    else:
        disp(i,"Radar position for bomb is "+cells[tar].p)
    return tar,noTarget
...
def make_end_point(myx,myy,tarx,tary):
    ...
    for di in range(len(dist)):
        if (dist[di] < mind):
            mind = dist[di]
            mini = di
    disp(i, "End point is "+str(tar[mini]))
    # print(cmd[mini])
    return tar[di]
...
def handle_moving_digging(i):
    if (target[i] == None):
        ...
        myx  = myrobots[i][0]
        myy  = myrobots[i][1]
        steppoint = make_single_step(i,make_end_point(myx,myy,tarx,tary))
        tarx = steppoint[0]
        tary = steppoint[1]
        disp(i,"Moving to: "+cells[xy2i(tarx,tary)].p)
        if (tarx == 0):
    ...
    for ib in range(width*height):
        if (cells[ib].blacklist):
            if (norm1(r[0]-cells[ib].x,r[1]-cells[ib].y) <= 1):
                ...
                thebombid = ib
    if (thebombid != None):
        disp(i,"There is a bomb nearby "+cells[thebombid].p)
        falive = 0
        ealive = 0
        ...
        if (ealive>1):
            if (nenemies>1)or((falive>ealive)and(nenemies>0)):
                nfriends = 0
                for f in myrobots:
...
def handle_killer_searsh(i):
    disttoenem = [[]]
    mx = myrobots[i][0]
    my = myrobots[i][1]

    for er1 in range(len(currentenemypoint)):
        for er2 in range(len(currentenemypoint)):
            p1 = currentenemypoint[er1]
            p2 = currentenemypoint[er2]
            disttoenem[er1].append(norm1(p1[0]-p2[0],p1[1]-p2[1]))
        disttoenem.append([])
    tar = None
    disttome = 1000
    for er1 in range(len(currentenemypoint)):
        for er2 in range(len(currentenemypoint)):
            if (er1 != er2) and (disttoenem[er1][er2] <= 2):
                p1 = currentenemypoint[er1]
                dist = norm1(p1[0]-mx,p1[1]-my)
                if (disttome > dist):
                    tar = [er1,er2]
    # disp(-1,"Enemy distances: "+str(disttoenem))
    if (tar != None):
        mx1 = currentenemypoint[tar[0]][0]
        my1 = currentenemypoint[tar[0]][1]
        mx2 = currentenemypoint[tar[1]][0]
        my2 = currentenemypoint[tar[1]][1]
        ty = None
        tx = None
        if (disttoenem[tar[0]][tar[1]] < 2):
            tx = mx1
            ty = my1
        else:
            if (mx1 == mx2):
...
def make_single_step(i,endpoint):
    tx = endpoint[0]
    ty = endpoint[1]
    mx = myrobots[i][0]
    my = myrobots[i][1]

    if (norm1(tx-mx,ty-my) <=4):
        return endpoint
        # for int in range(len(nextstep)):
        #     if (nextstep[int] != None):
        #         dist = norm1(tx-cells[nextstep[int]].x,ty-cells[nextstep[int]].y)
        #         if (dist <= 2):
        #             disp(-1,"Robots are to close "+str(i)+ " & "+str(int))
    else:
        indxs = [-4,-3,-2,-1,0,1,2,3,4]
        globdist = 1000
        tar = []
        nns = 0
        for ns in nextstep:
        ...
        for ix in indxs:
            for iy in indxs:
                px = mx+ix
                py = my+iy
                if (norm1(ix,iy) <= 4)and(px>=0)and(py>=0)and(px<30)and(py<15):
                    if (nns != len(nextstep)):
                        for ns in nextstep:
                            if (ns != None)and(norm1(px-cells[ns].x,py-cells[ns].y) > 2):
                                dist = norm1(px-tx,py-ty)
                                if (dist < globdist):
                                    globdist = dist
                                    tar = [px,py]
                    else:
        ...
        disp(i,str(tar))
        disp(i,"Next step is "+str(tar[0])+ " "+str(tar[1]))
        return tar
    ...
    if (gamecycle == 0):
        ...
        for ir in range(5):
            if abs(myrobots[ir][1] - 7) < dist:
                ...
                firstradarrunner = ir
                firstbombrunner = ir-1 if (firstradarrunner != 0) else ir+1
    ...
    for i in range(len(myrobots)):
        ...
        if (myx == -1) and (myy == -1):
            ...
            if (not handled) and (radar_cooldown == 0) \
                ...
                and (len(checkagain) < minCLAnewRadar) and (gamecycle < 175)\
                and (cantrickwait[i]):
            ...
            if (not handled):
                ...
                if ((tarx == -1) and (tary == -1)):
                    ...
                    if (dist > 1):
                        steppoint = make_single_step(i,make_end_point(myx,myy,tarx,tary))
                        # print_move(myx,myy,tarx,tary)
                        print("MOVE "+str(steppoint[0])+ " "+str(steppoint[1]))
                    else:
            ...
            if (not handled):
                ...
                if (noTarget):
                    if (trickwaitingbomb[i]) and (random.random() < 0.3):
                        tar,noTarget = get_bomb_position_deposit_trick(i)

                if(noTarget):
                    tar,noTarget = get_bomb_position_deposit(i,only_enemy_holes=True)
                    trickwaitingbomb[i] = False

                if (noTarget):
                    tar,noTarget = get_bomb_position_radar(i)
                    trickwaitingbomb[i] = False

                if (noTarget):
                    tar,noTarget = get_bomb_position_deposit(i,only_enemy_holes=False)
                    trickwaitingbomb[i] = False
